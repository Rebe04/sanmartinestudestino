...
export default function Edit({ placeCategory }) {
    const { data, setData, patch, processing, errors } = useForm({
        name: placeCategory.data.name || '',
    });

    const submit = (e) => {
        e.preventDefault();
        patch(route('admin.place-categories.update', placeCategory.data.slug));
    };

    return (
        <>
            <Head title={`Editar: ${placeCategory.data.name}`} />
            ...
            <div className="mt-6 max-w-2xl bg-white rounded-lg shadow-md p-6">
                <form onSubmit={submit}>
                    ...
                    <div className="mt-6 flex justify-end gap-4 py-4">
                        <Link href={route('admin.place-categories.index')} className="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50">Cancelar</Link>
