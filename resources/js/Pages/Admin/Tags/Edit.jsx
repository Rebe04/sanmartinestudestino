...
export default function Edit({tag}) {
    const { data, setData, patch, processing, errors } = useForm({
        name: tag.data.name || '',
        color: tag.data.color || '#f6c132',
    });

    const submit = (e) => {
        e.preventDefault();
        patch(route('admin.tags.update', tag.data.slug));
    };

    return (
        <>
            <Head title={`Editar: ${tag.data.name}`} />
...
Edit.layout = page => <AdminLayout children={page} title="Editar Tag" />;
